...
class FavoritesActivity() : AppCompatActivity(){
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val mechanicsButton = findViewById<Button>(R.id.options)
        ...
        * project, instead of we give information about what we want to do in the @CategoriesActivity
        * with the method putExtra() we are going to receive the information about what the
        * @CategoriesActivity want us to do. To give the information we have to use the putExtra()
        * method in the @CategoriesActivity and use the getXXXXExtra() method in this class that will
        * have the options chosen by the user in the @CategoriesActivity allowing us to reuse the same
        *  activity to do different tasks
        */
        val mechanicsIntent = Intent(this, CategoriesActivity::class.java)
